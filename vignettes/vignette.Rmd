...
knitr::opts_chunk$set(
  ...
  # remote bigwigs. Users running the vignette interactively can set
  # `knitr::opts_chunk$set(eval = TRUE)` or run individual chunks.
  eval = FALSE
)
```
...
```{bash, eval=FALSE}
R CMD INSTALL Seq2PlotR_0.1.0.tar.gz
```
...
Bigwigs are separated by strand, for unstranded data simply omit the '-' entry
```{r}
bigwigs = list(
  '+' = list(
    "TT-seq" = list(
...
For convenience one can assemble track information in an Excel sheet based on the template at \link{https://github.com/THJlab/Seq2PlotR/blob/master/inst/extdata/example_excel_template.xls} for an example.
